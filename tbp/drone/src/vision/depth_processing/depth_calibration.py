...
class DepthCalibrator:
    def __init__(self, image_path: str):
        ...
        self.depth_map, self.rgb_image = self.depth_estimator.estimate_depth(
            self.image_path
        )
    ...
    def click_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            depth_cm = float(
                input(
                    f"Enter true depth in cm for point ({x}, {y}), raw depth is {raw_depth:.2f}: "
                )
            )
    ...
    def run_calibration(self):
        ...
        self.current_display = np.hstack(
            (cv2.cvtColor(self.rgb_image, cv2.COLOR_BGR2RGB), depth_colored)
        )
