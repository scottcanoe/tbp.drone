...
class DepthEstimator:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        self.encoder = "vitb"
        self.dataset = (
            "hypersim"  # 'hypersim' for indoor model, 'vkitti' for outdoor model
        )
        ...
        if model_path is None:
            ...
            model_path = str(
                Path("~/tbp/tbp.drone/models/depth_anything_v2_vitb.pth").expanduser()
            )
    ...
    def estimate_depth(
        ...
        with torch.no_grad():
            depth_map = self.model.infer_image(
                rgb_image
            )  # outputs "affine invariant" INVERSE depth map
