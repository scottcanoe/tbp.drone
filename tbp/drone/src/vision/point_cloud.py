...
class DroneDepthTo3DLocations:
    ...
    def __call__(
        ...
        if (current_depth_h, current_depth_w) != self.resolution:
            modified_depth = cv2.resize(
                modified_depth, (self.resolution[1], self.resolution[0])
            )
            mask = (
                cv2.resize(
                    mask.astype(np.float32), (self.resolution[1], self.resolution[0])
                )
                > 0.5
            )
        ...
        if (orig_rgb_h, orig_rgb_w) != self.resolution:
            rgb_image_for_colors = cv2.resize(
                rgb_image_from_estimator, (self.resolution[1], self.resolution[0])
            )
        ...
        colors_normalized_rgb = (
            cv2.cvtColor(rgb_image_for_colors, cv2.COLOR_BGR2RGB) / 255.0
        )
        ...
        corners, ids, rejected = self.aruco_detector.detect(
            rgb_image_from_estimator, output_dir="."
        )
...
def process_spam_dataset_v2():
                ...
                fig_plotly = go.Figure(
                    data=[
                        ...
                        go.Scatter3d(
                            ...
                            z=z,
                            mode="markers",
                            marker=dict(size=2, color=point_colors_plotly, opacity=0.6),
                            name="Point Cloud",
                        ),
                        # Origin marker
                        go.Scatter3d(
    ...
    if all_points:
        ...
        plt.savefig(
            str(point_clouds_dir / "scatter3d_combined.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
            ...
            fig_plotly = go.Figure(
                data=[
                    ...
                    go.Scatter3d(
                        ...
                        z=combined_points[:, 2],
                        mode="markers",
                        marker=dict(size=2, color=combined_colors_plotly, opacity=0.6),
